...
module.exports = {
  ...
  feedback: function (req, res) {
    ...
    if (req.session.student.college_name == null || feedbacks == null || req.session.student.enrollment_no == null) {
      ...
      async.each(feedbacks, function (feedback, callback) {
        ...
        if (result.length == 15 && feedback.feedbackId != null) { // console.log("nothing");
            ...
            if (err)
              ...
              con.query(query2, [feedback.subject_code, feedback.instructor_code.toString()], function (err3, result3) {
                if (err3) {
                  ...
                  con.query(query3, [Number(req.session.student.enrollment_no)], function (err4, res4) {
                ...
                if (err3) {
                  ...
                  con.query(query3, [Number(req.session.student.enrollment_no)], function (err4, res4) {
                    if (err4) {
                      ...
                      console.log("Practical feedback id " + feedback.feedbackId + ' of length ' + result.length + ' updated ')
  ...
  studentData: function (req, res) {
    ...
    async.each(details.data, function (detail, callback) {
      ...
      if (detail["cells"][0]!='') {
        var enrollement_number = detail["cells"][0];
        var school = details.college;
        var course = details.course;
        var stream = details.stream;
        var year = parseInt(details.year, 10);
        var name = detail["cells"][1];
        var email = detail["cells"][2];
        var phone = detail["cells"][3];
        var password = detail["cells"][4];
        var student_table = school + "_student_" + year;
        var query0 = 'create table if not exists ' + student_table + '(enrollment_no bigint(20), name varchar(100), email varchar(100), phone varchar(100), year_of_admission int(4), password varchar(600), course varchar(100), stream varchar(100))';
        con.query(query0, function (err) {
          if (err) {
            ...
            con.query(query, [enrollement_number, name, email, phone, year, password, course, stream], function (err) {
              if (err) {
                console.error(err);
              } else {
                console.log(res);
  ...
  updateStudent: function (req, res) {
    ...
    if (course == null || stream == null || college == null || year == null) {
      ...
      year = year.toString();
      var query = "select * from " + college + "_student_" + year + " where" +
        " course='" + course + "' AND stream='" + stream + "'";
